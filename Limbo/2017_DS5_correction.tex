...
\iftrue
\page
\centerline{\bfb DEVOIR SURVEILLE 5 (CORRECTION)}
\blank[big]
...
\stopitemize
En conclusion, la proposition $\mc P_n$ est vraie pour $n∈ℕ$, de sorte que $T_n$ est un polynôme de degré $n$ et de coefficient dominant $2^n$. 
...
\NC 0\NC =g''(ϑ)=-\sin(ϑ)T_n(\cos ϑ)-\sin(ϑ)\cos(ϑ)T_n'(\cos ϑ)\NR\NC\NC-2\sin(ϑ)\cos(ϑ)T_n'(\cos ϑ)\NR
\NC\NC+\sin(ϑ)^3T_n''(\cos ϑ)+(n+1)\sin\big((n+1)ϑ\big)\NR
\NC\NC = -\sin(ϑ)T_n(\cos ϑ)-3\sin(ϑ)\cos(ϑ)T_n'(\cos ϑ)+\sin(ϑ)^3T_n''(\cos ϑ)\NR
\NC\NC+{\red (n+1)^2\sin\big((n+1)ϑ\big)}\qquad(0<ϑ<π)
}
\stopformula
...
\NC\NC= -T_n(\cos ϑ)+3\cos(ϑ)T_n'(\cos ϑ)-\sin(ϑ)^2T_n''(\cos ϑ)+{\red (n+1)^2T_n(\cos ϑ)}\NR
\NC\NC= \big (n+1)^2-1\big)T_n(\cos ϑ)+3\cos(ϑ)T_n'(\cos ϑ)-\sin(ϑ)^2T_n''(\cos ϑ)\NR
\NC\NC= (n^2+2n)T_n(\cos ϑ)+3\cos(ϑ)T_n'(\cos ϑ)-\sin(ϑ)^2T_n''(\cos ϑ)\qquad(0<ϑ<π)
}
\stopformula
\item Soit $n∈ℕ$. 
...
\startformula
\Align{
\NC u_{n+1}-u_n\NC =\int_0^{π\F 2}(\sin t)^{n+1}\d t-\int_0^{π\F 2}(\sin t)^n\d t\NR
\NC \NC =\int_0^{π\F 2}\Q((\sin t)^{n+1}-(\sin t)^n\W)\d t\NR
...
\stopformula
En conclusion, $X_2\hookrightarrow B(2pq)$.
...
\startformula
P(X_3=0)=1-3pq, \ P(X_3=1)=2pq\ P(X_3=2)=pq\ E(X_3)=4pq,\ V(X_3)=6pq-16p^2q^2
\stopformula
...
\NC \NC =∑_{k=1}^{n-1}∏_{j=1}^kP(P_j)×∏_{j=k+1}^nP(\overline{P_j})+∑_{k=1}^{n-1}∏_{j=1}^kP(\overline{P_j})×∏_{j=k+1}^nP(P_j)\NR
\NC \NC =∑_{k=1}^{n-1}p^k×q^{n-k}+∑_{k=1}^{n-1}q^k×p^{n-k}
}
\stopformula
...
∩_{j=1\atop j\text{ pair}}^n\overline{P_j}∩∩_{j=1\atop j\text{ impair}}^nP_j
}
\stopformula
...
P\Q(∩_{j=1\atop j\text{ pair}}^n\overline{P_j}∩∩_{j=1\atop j\text{ impair}}^nP_j\W)\NR
...
\startformula
\Align{
\NC P(X_4=1)\NC =2pq{q^{4-1}-p^{4-1}\F q-p}=2pq{q^3-p^3\F q-p}\NR
\NC\NC =2pq(q^2+pq+p^2)=2pq(1-2pq+pq)=2pq(1-pq)}
\stopformula
...
\NC \NC=1-4pq(p^2+q^2)-6p^2q^2\NR
\NC \NC= 1-4pq((p+q)^2-2pq)-6p^2q^2\NR
\NC\NC=1-4pq(1-2pq)-6p^2q^2=1-4pq+2p^2q^2
}
\stopformula
...
\startformula
P(X_4=0)=1-4pq+2p^2q^2, \ P(X_4=1)=P(X_4=2)=2pq(1-pq),\ P(X_4=3)=2(pq)^2
\stopformula
...
\startformula
\Align{
\NC P(X_3=0)=1-3pq=1-{3\F 4}={1\F 4}={2\choose 0}{1\F 2^2}\NR
\NC P(X_3=1)=2pq={1\F 2}={2\choose 1}{1\F 2}{1\F 2}, \NR
\NC P(X_3=2)=pq={1\F 4}={2\choose 2}{1\F 2^2}
}
\stopformula
...
\startformula
\Align{
\NC P(X_4=0)=1-4pq+2p^2q^2={1\F 8}={3\choose 0}{1\F 2^3},\NR 
\NC P(X_4=1)=P(X_4=2)=2pq(1-pq)={3\F 8}=\underbrace{{3\choose 1}}_{={3\choose 2}}{1\F 2^1}{1\F 2^2},\NR
\NC P(X_4=3)=2(pq)^2={1\F 8}={3\choose 3}{1\F 2^3}
}
\stopformula
En particulier, on reconnait que $X_3$ suit la loi binomiale $\mc B\Q(3,{1\F2}\W)$.
\item Soit $n⩾2$. 
\startList
\item Pour $1⩽k⩽n$, il résulte de l'indépendance du $n+1)\high{ième}$ tirage avec les $n$ premiers tirages que 
\startformula
\Align{
...
\startformula
\Align{
\NC P\big(X_n=k)∩(X_{n+1}=k)\big)\NC =P\big(X_n=k)∩(X_{n+1}=k)∩P_n∩P_{n+1}\big)\NR
\NC\NC +P\big(X_n=k)∩(X_{n+1}=k)∩P_n∩\overline{P_{n+1}}\big)\NR
\NC\NC +P\big(X_n=k)∩(X_{n+1}=k)∩\overline{P_n}∩P_{n+1}\big)\NR\NC\NC+P\big(X_n=k)∩(X_{n+1}=k)∩\overline{P_n}∩\overline{P_{n+1}}\big)\NR
\NC \NC =P\big(X_n=k)∩P_n∩P_{n+1}\big)+0\NR\NC\NC + 0+P\big(X_n=k)∩\overline{P_n}∩\overline{P_{n+1}}\big)\NR
\NC \NC ={1\F 2}P\big(X_n=k)∩P_n\big)+{1\F 2}P\big(X_n=k)∩\overline{P_n}\big)\NR
...
\stopformula
\startitemize[1]
\item Comme $X_2\hookrightarrow\mc B(1, {1\F2})=B\big({1\F 2}\big)=B({2pq})$, la proposition $\mc P_2$ est vraie.
\item Supposons la proposition $\mc P_n$ pour un entier $n⩾2$. 
Soit $k∈⟦1,n⟧$. Alors, il résulte de la formule de Pascal que 
\startformula
\Align{
