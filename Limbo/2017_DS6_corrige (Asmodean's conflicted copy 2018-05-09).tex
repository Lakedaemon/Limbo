...
\NC f(X³) \NC=f(f(X^3))=f(2X³-3X²)=2f(X^3)-3f(X^2)=2(2X^3-3X^2)-3(X^2-2X)=4X^3-9X^2+6\NR
}
\stopformula
...
$\dim\ker f^2=1$ et $\Vect( X-1)=\ker f⊂\ker f^2$ donc nécéssairement $\ker(f^2)=\Vect( X-1)$}
\stopList
\stopList
\blank[big]
...
\stopformula
De sorte que la $d$ définies par 
\startformula
d(x)=\System{
\NC α\e^{-{λ-1\F λ}\ln|x|}=|x|^{-{λ-1\F λ}}\NC \Si x∈ℝ^*
\NC 0\NC \Si x=0
}
\stopformula
est continues sur $ℝ$ (prolongement par continuité), paire et vérifie la relation $Φ(d)=λd$ comme on peut le vérifier en intégrant
\startformula
\Align{
...
\startformula
\Align{
\NC f\big(λ(x,y,z)+μ(x',y',z')\big)\NC =f(λx+μx',λy+μy',λz+μz')\NR
...
\startformula
{f(x)-f(0)\F x-0}={{1-\e^{-x}\F x}-1\F x}={1-x-\e^{-x}\F x^2}={1-x-\big(1-x+(-x)^2+o_0(x^2)\big)\F x^2}={-x^2+o_0(x^2)\F x^2}=-1+o_0(1)
\stopformula
...
\startformula
{\ln(n+1)\F \e}={1\F \e}\Q[\ln(1+u)\W]_0^n=\int_0^n{1\F\e}{1\F 1+u}\d u⩽\int_0^n{\e^{-{u\F n}}\F 1+u}\d u=u_n⩽\int_0^n{1\F 1+u}\d u = \Q[\ln(1+u)\W]_0^n=\ln(n+1)\qquad (n∈ℕ^*)
\stopformula
