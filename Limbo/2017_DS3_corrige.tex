...
\NC =\Q({1\F 2}\W)^n\Q({8\F 2^3}-{12\F 2^2}+{6\F 2}-1)=\Q({1\F 2}\W)^n(1-3+3-1)=0,
}
\stopformula
...
\NC =\Q({1\F 2}\W)^n\Q({8(n+3)\F 2^3}-{12(n+2)\F 2^2}+{6(n+1)\F 2}-n)=\Q({1\F 2}\W)^n(n+3-3(n+2)+3(n+1)-n)=\Q({1\F 2}\W)^n×0=0,
}
\stopformula
...
\NC =\Q({1\F 2}\W)^n\Q({8(n+3)^2\F 2^3}-{12(n+2)^2\F 2^2}+{6(n+1)^2\F 2}-n^2)=\Q({1\F 2}\W)^n\underbrace{\big((n+3)^2-3(n+2)^2+3(n+1)^2-n^2)}_{(1-3+3-1)^2+n(6-12+6)+3^2-3×2^2+3}=\Q({1\F 2}\W)^n×0=0,
}
\stopformula
...
\startformula
\System{
\NC 0=xi_0+yj_0+zk_0=x+y×0+z×0^2=x\NR
\NC 0=xi_1+yj_1+zk_1={x\F 2}+{y\F 2}×1+{z\F 2}×1^2={x\F 2}+{y\F 2}+{z\F 2}\NR
\NC 0=xi_2+yj_2+zk_2={x\F 4}+{y\F 4}×2+{z\F 4}×2^2={x\F 4}+{2y\F 4}+{4z\F 4}\NR
}⟺\System{
\NC 0=x\NR
\NC 0=y+z\NR
\NC 0=2y+4z
...
\startformula
\Align{
\NC S_{n-1}\NC =∑_{k=0}^{n-1}(2^{k+1}u_{k+1}-2^ku_k)\NR
\NC \NC = ∑_{k=0}^{n-1}2^{k+1}u_{k+1}-∑_{k=0}^{n-1}2^ku_k\qquad\text{(séparation)}\NR
\NC \NC = ∑_{ℓ=1}^n2^ℓu_ℓ-∑_{k=0}^{n-1}2^ku_k\qquad\text{(changement d'indice  $ℓ=k+1$)}\NR
\NC \NC = ∑_{ℓ=1}^{n-1}2^ℓu_ℓ+2^nu_n-2^0u_0-∑_{k=1}^{n-1}2^ku_k\qquad\text{(Chasles)}\NR
\NC \NC = 2^nu_n-u_0
\stopformula
...
\startformula
\Align{
\NC u_n\NC ={u_0\F 2^n} + λ{n\F 2^n}+μ{n^2-n\F 2^{n+1}\qquad(n⩾1)\NR
\NC \NC = u_0 {1\F 2^n}+ \Q(λ-{μ\F 2}\W){n\F 2^n}+ {μ\F 2}{n^2\F 2^n}\NR
\NC\NC =  u_0i_n+ \Q(λ-{μ\F 2}\W)j_n+ {μ\F 2}k_n\qquad(n⩾1)
}
\stopformula
...
\stopformula
Ainsi, $\vec u$ est $\vec i$, $\vec j$ et $\vec k$.
\stopList%A3
\item La famille $(\vec i, \vec j, \vec k)$ est une famille libre de $E$ d'après 1.2b et génératrice de $E$ d'après 1.3b 
puisque   toute suite $\vec u$ de $E$ est combinaison linéaire des suites $\vec i$, $\vec j$ et $\vec k$. 
A fortiori, c'est une base de $E$.
...
\NC\NC =2^k\Q(\underbrace{8u_{k+3}-12u_{k+2}+6u_{k+1}-u_k}_{0\text{ car $u∈E$}}=0}
\stopformula
...
x^2-2x+1=0⟺(x-1)^2=0,
\stopformula
admettant la racine double $1$. En particuier, il existe un unique couple $(λ,μ)∈ℝ^2$ tel que 
\startformula
...
\startformula
\Align{
\NC P_{U_1}(R_k)={2\F 5},NR
\NC P_{U_2}(R_k)={1\F 1}=1\NR
\NC P_{U_3}(R_k)={0\F 1}=0
}
\stopformula
...
\NC P_{U_3}(R_1∩⋯∩R_n)=P_{U_3}(R_1)×⋯×P_{U_3}(R_n)=0^n=0
}
\stopformula
...
\startformula
\Align{
\NC P(R_1∩⋯∩R_n)\NC =P(U_1)×P_{U_1}(R_1∩⋯∩R_n)+P(U_2)×P_{U_2}(R_1∩⋯∩R_n)+P(U_3)×P_{U_3}(R_1∩⋯∩R_n)\NR
...
\startformula
\startAlign{
\NC P(A_k)\NC =P(R_1)×P_{R_1}(R_1∩R_2)×⋯×P_{R_1∩⋯∩R_{k-1}}(R_k)\NR
\NC \NC = {7\F 15}×∏_{ℓ=2}^kP_{R_1∩⋯∩R_{ℓ-1}}(R_ℓ)\NR
\NC\NC = {7\F 15}×∏_{ℓ=2}^k{\Q({2\F 5}\W)^ℓ + 1\F \Q({2\F 5}\W)^{ℓ-1}+1}
\NC\NC = {7\F 15}×{\Q({2\F 5}\W)^k+1\W)\F \Q({2\F 5}\W)^1+1}\NR
\NC\NC = {7\F 15}×{\Q({2\F 5}\W)^k+1\W)\F {7\F 5}}\NR
\NC\NC = {1\F 3}×\Q({2\F 5}\W)^k+1\W)
}
\stopformula
...
\startformula
\align{
\NC P(A)\NC =1-P(∪_{k=1}^nA_k)=1-∑_{k=1}^nP(A_k)\qquad\text{(événements mutuellemens incompatibles)}\NR
\NC \NC =1-∑_{k=1}^n{1\F 3}×\Q({2\F 5}\W)^k+1\W)\NR
\NC\NC =1-{n\F 3}-{1\F 3}{\Q({2\F 5}\W)^1-\Q({2\F 5}\W)^{n+1}\F 1-{2\F 5}}
}
\stopformula
\stopList%Bl2
\stopList%L1
...
\startformula
h'(t)={\gray \underbrace{ 0⩽}_{\text{ne servira pas}}}\e^t⩽{\red 1\F \e}\qquad(α_2⩽t⩽-1)
\stopformula
Il résulte alors de l'inégalité des accroissements finis appliquée entre $x∈[α_2,-1]$ et $α_2$ que 
\startformula
{\gray 0=0×(x-α_2)}⩽h(x)-α_2⩽{\red 1\F\e}(x-α_2)\qquad(α_2⩽x⩽-1)
\stopformula
...
\startformula
\Align{
\NC {0⩽}_{\text{d'après 3b}}u_{k+1}-α_2\NC \underbrace{⩽}_{\text{d'après 3d}}{ 1\F\e}(u_k-α_2)`=\underbrace{⩽}_{\text{d'après $\mc P_k$}}{ 1\F\e}\Q({1\F \e}\W)^k\NR
\NC\NC ⩽\Q({1\F \e}\W)^{k+1}
}
\stopformula
Donc $\mc P_{k+1}$ est vraie.
\item Il résulte de la question précédente et du principe des gendarmes que la suite $u$ converge vers $α_2$.
\stopitemize
\stopList%3L
\stopList%L1
\stopList