...
\item Modifier le programme pour qu'il compte et affiche combien de fois $w=2u$. 
...
\item Ecrire un programme demandant à l'utilisateur le nombre de questions $n$ qu'il pense avoir justes à ce devoir et affichant :
\startitemize[1]
\item \quote{pessimiste !} pour $n⩽3$
\item \quote{réaliste ?} pour $4⩽n<8$
\item \quote{optimiste !} pour $n⩾11$
\item \quote{attentif ?} sinon
\stopitemize

% input, for 
\item Ecrire un programme demandant des entiers $n$ et $p$ à l'utilisateur, puis affichant la valeur de la somme $∑_{k=0}^nk^p$. 

% while
\item La suite récurrente définie par $u_0={1\F 2}$ et $u_{n+1}= u_n-u_n^2$ pour $n⩾0$ est décroissante 
et tends vers $0$. Ecrire un programme affichant le premier indice pour lequel $u_n⩽ {1\F 10}$ et la valeur $u_n$ correspondante.
